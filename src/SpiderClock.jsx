...
function SpiderClock({ onClose, showToast }) {
  const containerRef = useRef(null);
  const watchRef = useRef(null);

  // Position state for the draggable elastic spider clock
  const defaultY = 60;
  const [pos, setPos] = useState({ x: 0, y: defaultY });
  const posRef = useRef({ x: 0, y: defaultY });
  const isDragging = useRef(false);
  const dragStart = useRef({ mouseX: 0, mouseY: 0, startX: 0, startY: defaultY });
  ...
  // Elastic Drag & Release Spring Handlers
  ...
  const handleMouseDown = (e) => {
    if (e.target.closest('.spider-clock-close-btn')) return;
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      // Allow pulling left/right and pulling down (limit minimum Y to 15px)
      const newX = dragStart.current.startX + dx;
      const newY = Math.max(15, dragStart.current.startY + dy);
    ...
    const handleEndDrag = () => {
      ...
      // Elastic Spring Bounce back up to original resting position
      const animTarget = { x: posRef.current.x, y: posRef.current.y };
      tweenRef.current = gsap.to(animTarget, {
        x: 0,
        y: defaultY,
        duration: 1.6,
        ease: "elastic.out(1.2, 0.35)",
        onUpdate: () => {
          setPos({ x: animTarget.x, y: animTarget.y });
        },
      });

      if (showToast && Math.abs(posRef.current.y - defaultY) > 80) {
        showToast('🕷️ বোয়িং! স্পাইডার জালের সুতোয় বাউন্স করে ওপরে উঠে এসেছে!');
      }
    };
  ...
  return (
    <div className="spider-clock-container" ref={containerRef}>
      {/* Interactive Elastic Silk Web Line (SVG from ceiling top down to clock center) */}
      <svg
        style={{
          position: 'absolute',
          top: 0,
      ...
      >
        {onClose && (
          <button
            onClick={onClose}
            className="spider-clock-close-btn"
            title="Close Spider Clock"
          >
            ✕
          </button>
        )}
        ...
        <div id="wBody">
          <div id="watch">
            ...
            >
              {/* Semi-Transparent Frosted Dial Backdrop */}
              <circle
                cx="150"
                cy="150"
                r="147"
                fill="url(#spiderDialGradient)"
                stroke="#D4AF37"
                strokeWidth="3"
              />
              <circle
              ...
              <g transform="matrix(0.97,0.02,-0.02,0.97,19.19,-6.07)">
                <use className="cog cw t24" fill="#94A3B8" fillOpacity="0.4" href="#cog1" />
              </g>
              <use className="cog ccw t12" fill="#94A3B8" fillOpacity="0.4" href="#cog2" />
              <g transform="matrix(0.93,0,0,0.93,9.460262,5.75)">
                <use
                  className="cog cw t24"
                  fill="#94A3B8"
                  fillOpacity="0.4"
                  href="#cog1"
              ...
              <g transform="rotate(-3.7621103,160.78969,349.2265)">
                <use className="cog cw t20" fill="#94A3B8" fillOpacity="0.4" href="#cog5" />
              </g>

              <g transform="rotate(-2.7463672,162.45282,88.573956)">
                <use
                  className="cog ccw t12"
                  href="#cog2"
                  fill="#94A3B8"
                  fillOpacity="0.5"
              ...
              {/* HIGH CONTRAST LUMINOUS GOLD NUMERALS */}
              <use
                id="clockFace"
                fill="#FFE066"
                fillOpacity="1.0"
        ...
        {/* Hover drag hint */}
        <div className="spider-clock-hint">
          🕷️ টান দিয়ে নিচে নামান (ছেড়ে দিলে স্প্রিং করবে)
        </div>
      </div>

      {/* SVG DEFS FROM ORIGINAL SOURCE WITH ENHANCED DIAL GRADIENT */}
      <svg width="0" height="0" role="none" style={{ position: 'absolute', pointerEvents: 'none' }} xmlns="http://www.w3.org/2000/svg">
        <defs>
          <radialGradient id="spiderDialGradient" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="#2A303C" stopOpacity="0.88" />
            <stop offset="70%" stopColor="#181C24" stopOpacity="0.92" />
            <stop offset="100%" stopColor="#0B0D12" stopOpacity="0.95" />
          </radialGradient>

          <filter id="shadowGold" x="-15%" y="-15%" width="130%" height="130%">
            <feDropShadow dx="0" dy="0" floodColor="#000000" floodOpacity="0.9" stdDeviation="1.5" />
            <feDropShadow dx="0" dy="0" floodColor="#FFD700" floodOpacity="0.4" stdDeviation="0.8" />
          </filter>
          <filter id="shadow" x="-5%" y="-5%" width="110%" height="110%">
            <feDropShadow dx="0" dy="0" floodColor="#FFFFFF" floodOpacity="0.4" stdDeviation="1.2" />
          </filter>
          <filter id="shadow2" x="-25%" y="-25%" width="150%" height="150%">
            <feDropShadow dx="0" dy="0" floodColor="#000000" floodOpacity="0.8" stdDeviation="1.5" />
          </filter>
          <filter id="blur1">
