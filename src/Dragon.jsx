...
export default function Dragon({ opacity = 0.7, size = 1.0 }) {
  const screenRef = useRef(null);
  const sizeRef = useRef(size);

  // Keep sizeRef in sync with the size prop for the animation loop
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const prepend = (useId, i) => {
      ...
      elems[i].use = elem;
      elem.setAttribute("href", "#dragon-" + useId);
      screen.prepend(elem);
    };
    ...
    const run = () => {
      ...
      for (let i = 1; i < N; i++) {
        ...
        const s = ((162 + 4 * (1 - i)) / 50) * sizeRef.current;
        e.use.setAttribute(
          "transform",
    ...
    return () => {
      ...
      // Remove all injected <use> elements to prevent ghost dragons on remount
